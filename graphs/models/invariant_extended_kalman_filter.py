...
from utils.constant_utils import TENSOR_EYE3, TENSOR_EYE12, TENSOR_EYE21, TIMESTAMP, FILTER_NAV_ROTATION_MATRIX, \
    FILTER_NAV_VELOCITY, FILTER_NAV_POSITION, FILTER_IMU_GYROSCOPE_BIAS, FILTER_IMU_ACCELEROMETER_BIAS, FILTER_CAR_ROTATION_MATRIX, FILTER_CAR_POSITION, FILTER_STATE_COVARIANCE, \
    FILTER_NOISE_COVARIANCE, FILTER_MEASUREMENT_COVARIANCE, TENSOR_EYE6
...
class InvariantExtendedKalmanFilter(nn.Module):
    ...
    def forward(self, sequence_data):
        ...
        predicted_data_dic = {
            ...
            FILTER_IMU_GYROSCOPE_BIAS: self.state_imu_gyroscope_bias,
            FILTER_IMU_ACCELEROMETER_BIAS: self.state_imu_accelerometer_bias,
            FILTER_CAR_ROTATION_MATRIX: self.state_car_rotation_matrix,
            FILTER_CAR_POSITION: self.state_car_position,
            FILTER_STATE_COVARIANCE: self.state_covariance,
            FILTER_NOISE_COVARIANCE: self.noise_covariance,
            FILTER_MEASUREMENT_COVARIANCE: self.measurement_covariance
        }
    ...
    def load_filter(self, model_file_name):
        if self.device == 'cpu':
            self.load_state_dict(torch.load(InvariantExtendedKalmanFilter.get_model_path(model_file_name), map_location='cpu'))
        else:
    ...
    def filter_update(self, i, propagated_state):
        ...
        propagate_state_car_velocity = propagated_state[
            InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_ROTATION_MATRIX].t().mv(
            propagate_state_imu_velocity + skew(propagate_measurement_gyroscope).mv(
                propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_POSITION]))
    ...
    #
    #     h_function_car_velocity = propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_ROTATION_MATRIX].t().mm(skew(propagate_state_imu_velocity
    #                                         + skew(propagate_measurement_gyroscope).mv(propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_POSITION])))
    #
    #     h_function_car_position = -propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_ROTATION_MATRIX].t().mm(skew(propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_POSITION]))
    #     filter_function_h = torch.zeros(3, self.state_dim, dtype=torch.float64).to(self.device)
    #
    #     filter_function_h[:, 3:6] = propagate_state_nav_car_rotation_matrix.t()
    #
    ...
    #
    #     filter_function_h[:, 15:18] = h_function_car_velocity
    #     filter_function_h[:, 18:21] = - propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_ROTATION_MATRIX].t().mm(propagate_measurement_gyroscope_skew)
    #
    #     measurement_covariance_r = torch.diag(self.measurement_covariance[i, :]).double().to(self.device)
    #     filter_function_s = filter_function_h.mm(propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_COVARIANCE]).mm(filter_function_h.t()) + measurement_covariance_r
    #     filter_gain_k_t = torch.linalg.solve(filter_function_s, propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_COVARIANCE].mm(filter_function_h.t()).t())
    ...
    def filter_loop_improved(self):
        for i in range(1, self.sequence_len):
            propagated_state = self.filter_propagate_improved(i)
            update_state = self.filter_update_improved(i, propagated_state)
    ...
    def filter_propagate_improved(self, i):
        base_state_nav_rotation_matrix = self.state_nav_rotation_matrix[i - 1, :, :].clone()
        ...
        f_noise_jacobian_matrix = torch.zeros(self.state_dim, self.noise_dim, dtype=torch.float64).to(self.device)
        f_noise_jacobian_matrix[:9, :6] = f_state_jacobian_matrix[:9, 9:15]
        f_noise_jacobian_matrix[9:15, 6:12] = TENSOR_EYE6
        f_noise_jacobian_matrix[15:18, 12:15] = self.state_car_rotation_matrix[i - 1, :, :].t()
        f_noise_jacobian_matrix[18:21, 15:18] = TENSOR_EYE3
        f_noise_jacobian_matrix = f_noise_jacobian_matrix * self.measurement_delta_time[i - 1]
    ...
    def filter_update_improved(self, i, propagated_state):
        propagate_measurement_gyroscope = self.measurement_gyroscope[i - 1, :] - propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_IMU_GYROSCOPE_BIAS]
        propagate_measurement_gyroscope_skew = skew(propagate_measurement_gyroscope)

        propagate_state_imu_velocity = propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_NAV_ROTATION_MATRIX].t().mv(
            propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_NAV_VELOCITY])
        propagate_state_car_velocity = propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_ROTATION_MATRIX]\
            .mv(propagate_state_imu_velocity + skew(propagate_measurement_gyroscope).mv(propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_POSITION]))

        h_function_nav_velocity = propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_ROTATION_MATRIX]\
            .mm(propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_NAV_ROTATION_MATRIX].t())
        h_function_imu_gyroscope_bias = propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_ROTATION_MATRIX]\
            .mm(skew(propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_POSITION]))
        h_function_car_rotation = -skew(propagate_state_car_velocity)
        h_function_car_position = propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_CAR_ROTATION_MATRIX]\
            .mm(propagate_measurement_gyroscope_skew)
        filter_function_h = torch.zeros(3, self.state_dim, dtype=torch.float64).to(self.device)
        filter_function_h[:, 3:6] = h_function_nav_velocity
        filter_function_h[:, 9:12] = h_function_imu_gyroscope_bias
        filter_function_h[:, 15:18] = h_function_car_rotation
        filter_function_h[:, 18:21] = h_function_car_position
        measurement_covariance_r = torch.diag(self.measurement_covariance[i, :]).double().to(self.device)
        filter_function_s = filter_function_h.mm(propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_COVARIANCE]).mm(filter_function_h.t()) + measurement_covariance_r
        filter_gain_k_t = torch.linalg.solve(filter_function_s, propagated_state[InvariantExtendedKalmanFilter.PROPAGATE_STATE_COVARIANCE].mm(filter_function_h.t()).t())
        filter_gain_k = filter_gain_k_t.t()
